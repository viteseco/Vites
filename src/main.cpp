...
const string strMessageMagic = "vitescoin very Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // vitescoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // vitescoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// vitescoin: find block wanted by given orphan block
...
// miner's coin stake reward based on coin age spent (coin-days) COPYRIGHT VITESCOIN 2016 DEVELOPERS
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = 0 * COIN;

    if(pindexBest->nHeight < 260800)
    {
...
// vitescoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // vitescoin: target change every block
    // vitescoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    // fMiner is true when called from the internal vitescoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // vitescoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // vitescoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // vitescoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // vitescoin: track money supply and mint amount info
...
// vitescoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const
{
    ...
    CBigNum bnCoinDay = bnCentSecond * CENT / COIN / (24 * 60 * 120);
...
// vitescoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // vitescoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // vitescoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // vitescoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // vitescoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // VITEScoin: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // vitescoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // vitescoin: check proof-of-stake
    ...
    // vitescoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // vitescoin: getblocks may not obtain the ancestor block rejected
    ...
    // vitescoin: if responsible for sync-checkpoint send it
...
// vitescoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "vitescoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
...
#if 0
    // Set up the VITEScoin Params object
    ZCParams = new libVITEScoin::Params(bnTrustedModulus);
#endif
    ...
    //  TestNet integration into VITEScoin - plainkoin
    //
	...
	if(!fTestNet)
	{
        ...
        const char* pszTimestamp = "a token dies and a coin is born";
        //CTransaction txNew;
        txNew.nTime = 1532314870;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        {
        const char* pszTimestamp = "Tokens are bad - Testnet";
        //CTransaction txNew;
        txNew.nTime = 1532066418;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0 << CBigNum(42) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].SetEmpty();
	...
	uint256 actualGenesisBlock = !fTestNet ? hashGenesisBlock : hashGenesisBlockTestNet;
        ...
        //// debug print
         ...
         if(!fTestNet)
        {
            assert(block.hashMerkleRoot == uint256("0x996c9bb75af3f8ef5c6033ac1644f5caba35eb3e0295376de979cd6b000d41ec"));
        }
        else
        {
            assert(block.hashMerkleRoot == uint256("0x996c9bb75af3f8ef5c6033ac1644f5caba35eb3e0295376de979cd6b000d41ec"));
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // vitescoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // vitescoin: tell downloading node about the latest block if it's
