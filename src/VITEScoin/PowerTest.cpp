...
* @file       Tests.cpp
*
* @brief      Test routines for VITEScoin.
*
...
Test_GenerateGroupParams()
{
	...
	for (count = 0; count < 1; count++) {
		...
		try {
			group = deriveIntegerGroupParams(calculateSeed(GetTestModulus(), "test", VITEScoin_DEFAULT_SECURITYLEVEL, "TEST GROUP"), pLen, qLen);
...
Test_ParamGen()
{
	...
	try {
		...
		Params testParams(GetTestModulus(),VITEScoin_DEFAULT_SECURITYLEVEL);
...
Test_RunAllTests()
{
	printf("VITEScoin v%s self-test routine\n", VITEScoin_VERSION_STRING);
	...
	// Run through all of the VITEScoin tests
