...
// vitescoin: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-vitescoin-address
...
// VITEScoin: get current stake weight
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Calculate coin age reward
    {
        ...
        {
            if (fDebug && GetBoolArg("-printcoinstake"))
                    printf("CreateCoinStake : stake POS reward is 0 VITES\n");
            return false;
        }
        if (fDebug && GetBoolArg("-printcoinstake"))
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CvitescoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CvitescoinAddress(address).ToString());
}
...
// vitescoin: check 'spent' consistency between wallet and txindex
// vitescoin: fix wallet spent state according to txindex
...
// vitescoin: disable transaction (only for coinstake)
