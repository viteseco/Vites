...
{
    QString ret = QString("vitescoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non VITES unit input in lnReqAmount, we generate the URI with VITES as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(vitescoinUnits::format(vitescoinUnits::VITES, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
